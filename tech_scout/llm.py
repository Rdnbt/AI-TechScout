"""
LLM Client Module for AI-TechScout

This module provides utilities for interacting with various Large Language Models
including Anthropic Claude, OpenAI GPT, DeepSeek, and Google Gemini.
"""
...
def get_response_from_llm(
    ...
    Returns:
        Tuple of (response_content, updated_msg_history)
    """
...
def get_batch_responses_from_llm(
    ...
    Returns:
        Tuple of (list of response contents, list of msg histories)
    """
    if msg_history is None:
        msg_history = []
    ...
    if 'gpt' in model:
        ...
        # For models that don't support n > 1, make multiple calls
...
def extract_json_between_markers(llm_output: str) -> Optional[Any]:
    ...
    Looks for JSON between ```json and ``` markers, or falls back to
    finding JSON-like content in the output.
...
def extract_list_json_between_markers(llm_output: str) -> Optional[List]:
    """
    ...
    Returns:
        Parsed JSON list, or None if no valid list found
    """
